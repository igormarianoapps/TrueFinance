      ...
      <Card className="bg-white dark:bg-[#1F1F1F]">
        <div className="p-6">
          ...
          {profile?.subscription_status === 'active' ? (
            <button onClick={handleManageSubscription} disabled={loadingPortal} className="w-full bg-[var(--primary)] dark:bg-[#2A2A2A] text-white py-3 rounded-xl font-bold shadow-lg active:scale-95 transition-transform disabled:opacity-50">
              {loadingPortal ? 'Carregando...' : 'Gerenciar Assinatura'}
            </button>
          ) : (
            <button onClick={() => setShowPaywall(true)} className="w-full bg-[var(--primary)] text-white py-3 rounded-xl font-bold shadow-lg active:scale-95 transition-transform">
      ...
      <Card className="bg-white dark:bg-[#1F1F1F]">
        <div className="p-6">
          ...
          <form onSubmit={handleUpdateName} className="space-y-4">
            ...
            <button type="submit" disabled={loadingName || uploading} className="w-full bg-[var(--primary)] dark:bg-[#0B0C0C] text-white py-3 rounded-xl font-bold shadow-lg active:scale-95 transition-transform disabled:opacity-50">{loadingName ? 'Salvando...' : 'Salvar Nome'}</button>
          </form>
        </div>
      </Card>
      ...
      <Card className="bg-white dark:bg-[#1F1F1F]">
        <div className="p-6">
          ...
          <div className="flex gap-2">
            <button onClick={() => setTheme('light')} className={`flex-1 flex flex-col items-center gap-2 p-3 rounded-lg border-2 transition-colors ${theme === 'light' ? 'border-[var(--primary)] dark:border-[#0B0C0C] bg-blue-50 dark:bg-[#2A2A2A]' : 'border-transparent bg-slate-100 dark:bg-[#2A2A2A] hover:bg-slate-200 dark:hover:bg-[#333]'}`}>
              <Sun size={20} />
              <span className="text-sm font-semibold">Claro</span>
            </button>
            <button onClick={() => setTheme('dark')} className={`flex-1 flex flex-col items-center gap-2 p-3 rounded-lg border-2 transition-colors ${theme === 'dark' ? 'border-[var(--primary)] dark:border-[#0B0C0C] bg-blue-50 dark:bg-[#2A2A2A]' : 'border-transparent bg-slate-100 dark:bg-[#2A2A2A] hover:bg-slate-200 dark:hover:bg-[#333]'}`}>
              <Moon size={20} />
              <span className="text-sm font-semibold">Escuro</span>
            </button>
            <button onClick={() => setTheme('system')} className={`flex-1 flex flex-col items-center gap-2 p-3 rounded-lg border-2 transition-colors ${theme === 'system' ? 'border-[var(--primary)] dark:border-[#0B0C0C] bg-blue-50 dark:bg-[#2A2A2A]' : 'border-transparent bg-slate-100 dark:bg-[#2A2A2A] hover:bg-slate-200 dark:hover:bg-[#333]'}`}>
              <Monitor size={20} />
