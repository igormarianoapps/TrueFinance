    ...
    <div className="space-y-4 pb-20 animate-in slide-in-from-right-4">
      <div className="grid grid-cols-2 gap-3">
        {filteredData.tags.map(tag => (
          <div key={tag.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex flex-col items-center justify-center gap-2 relative group hover:shadow-md transition-shadow">
            <div className="w-8 h-8 rounded-full" style={{ backgroundColor: tag.cor }}></div>
            <span className="font-semibold text-slate-700">{tag.nome}</span>
