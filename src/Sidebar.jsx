    ...
    <>
      ...
      <div className={`fixed top-0 left-0 h-full w-72 bg-white z-50 shadow-2xl transform transition-transform duration-300 ease-out flex flex-col ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 flex justify-between items-center border-b border-slate-100 bg-slate-50">
          <div>
            <h2 className="font-bold text-xl text-slate-600">True Finance</h2>
        ...
        <nav className="p-4 space-y-2 flex-1">
          {menuItems.map(item => (
            <button
              ...
              className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all ${activeTab === item.id ? 'bg-gradient-to-r from-slate-900 to-violet-700 text-white shadow-md shadow-violet-200' : 'text-slate-500 hover:bg-slate-100'}`}
            >
              <item.icon size={20} />
