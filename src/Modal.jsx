            ...
            <div className="bg-slate-50 dark:bg-[#2A2A2A] p-3 rounded-lg border border-slate-100 dark:border-[#333]">
              ...
              <div className="flex gap-2 mb-3">
                {['unico', 'mensal', 'parcelado'].map(type => (
                  <button type="button" key={type} onClick={() => setRecurrenceType(type)} className={`flex-1 py-2 text-xs font-bold rounded-md transition-all ${recurrenceType === type ? 'bg-[#3457A4] dark:bg-[#0B0C0C] text-white shadow-md' : 'bg-white dark:bg-[#1F1F1F] text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-[#333]'}`}>
               ...
               <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                  {data.tags.map(tag => (
                    <label key={tag.id} className="cursor-pointer">
                      ...
                      <div className="px-3 py-1 rounded-full text-sm border peer-checked:bg-[#3457A4] dark:peer-checked:bg-[#0B0C0C] peer-checked:text-white peer-checked:border-[#3457A4] dark:peer-checked:border-[#0B0C0C] transition-all text-slate-600 dark:text-slate-300 border-slate-200 dark:border-[#333] hover:bg-slate-50 dark:hover:bg-[#2A2A2A]">
                        #{tag.nome}
                      </div>
                    </label>
                  ))}
